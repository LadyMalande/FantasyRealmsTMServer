...
class DeletesAllTypeTest {
    ...
    void initHand(){
        ...
        cardToBeTested = deck.stream().filter(card -> card.getNameLoc("cs").equals("Sněhová vánice")).findAny().get();
    ...
    void countConditionSatisfied() {
        ResourceBundle rb = ResourceBundle.getBundle("server.CardNames");
        Card c = deck.stream().filter(card -> card.getNameLoc("cs").equals("Stoletá voda")).findAny().get();
        cardsInHands.add(c);
