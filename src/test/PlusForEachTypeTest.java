...
class PlusForEachTypeTest {
    ...
    void initHand(){
        ...
        cardToBeTested = deck.stream().filter(card -> card.getNameLoc("cs").equals("Kovárna")).findAny().get();
    ...
    void countConditionSatisfied2() {
        ...
        Bonus bonus = cardToBeTested.getBonuses().get(0);
        assertEquals(bonus.count(cardsInHands), 18);
    }
    ...
    void countConditionSatisfied3() {
        ...
        cardsInHands.add(deck.stream().filter(card -> card.getNameLoc("cs").equals("Válečná loď")).findAny().get());
        Bonus bonus = cardToBeTested.getBonuses().get(0);
